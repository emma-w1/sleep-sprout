...
def everydayAnalysis(): #YOU WILL HAVE TO CREATE PARAMETERS AND RUN THIS EVERYTIME FIREBASE IS UPDATEDD

    def extract_total_values(key):
        total_values = []
        if isinstance(all_data, dict):
            ...
            for doc_key, doc_data in all_data.items():
                # Check if "numMoved" exists within the current document's data
                if isinstance(doc_data, dict) and key in doc_data:
                    total_values.append(doc_data[key])
        return total_values

    all_moved = extract_total_values("numMoved")
    all_temps = extract_total_values("temptoGive")
    all_snores = extract_total_values("numSnore")
    ...
    hours = duration/60
    # avg moves in moves/hr
    avg_moves = total_moves/hours
    # avg snores in snores/hour
    avg_snores = total_snores/hours
    ...
    def grow_decision(classifications,duration):
        ...
        final_score = duration_score(.4) + classification_score(.6) 
    ...
    print(grow_decision(classification_list,duration))
